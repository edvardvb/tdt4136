...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def value(self, gameState, counter):
        if counter == self.depth * gameState.getNumAgents()\
                or gameState.isWin()\
                or gameState.isLose():
            return (self.evaluationFunction(gameState), None)
        if counter % gameState.getNumAgents() == 0:
            return self.getMaxChildren(gameState, counter)
        else:
            return self.getMinChildren(gameState, counter)

    def getMaxChildren(self, gameState, counter):
        val = (-float('inf'), None)
