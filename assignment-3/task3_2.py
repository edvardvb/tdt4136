...
def a_star_loop(open_nodes, closed_nodes, cells, alg_type="A"):
    done = False
    while not done:
        if not open_nodes:
            break  # failure
...
if a_star_loop(open_nodes, closed_nodes, cells, "A"):
    ...
    print(lines)
    print(total)
    draw_path(lines, width, height, open_nodes, closed_nodes)
else:
    print('Failed')
